...
class Interpreter:
    ...
    def __init__(self, debug: bool=False, verbose: bool=False, performance_metrics: bool=False):
        self.debug = debug
                    ...
                    case _:
                        if line.startswith(":"):
                            if strip_variable_sigil(line) in self.global_variables:
                                print(f"{strip_variable_sigil(line)}: {self.global_variables[strip_variable_sigil(line)]}")
                            else:
    ...
    def _evaluate_node(self, node: Any, local_vars: dict[str, Any] = {}) -> Any:
        if self.verbose:
            print("Evaluating node: ", node, "Local variables: ", local_vars, "Global variables: ", self.global_variables)
        ...
        match node:
            case ["+", left, right]:
                return self._evaluate_node(left, local_vars) + self._evaluate_node(right, local_vars)
            case ["-", left, right]:
                return self._evaluate_node(left, local_vars) - self._evaluate_node(right, local_vars)
            case ["*", left, right]:
                return self._evaluate_node(left, local_vars) * self._evaluate_node(right, local_vars)
            ...
            case ["defn", name, value] if name.startswith(":"):
                ...
                self.global_variables[strip_variable_sigil(name)] = Variable(self._evaluate_node(value, local_vars), is_local=False)
            case ["defn", name, value] if name.startswith("$"):
                if strip_variable_sigil(name) in local_vars:
                    raise Exception(f"Variable {name} is already defined")
                local_vars[strip_variable_sigil(name)] = Variable(self._evaluate_node(value, local_vars), is_local=True)
            case ["defn-mutable", name, value] if name.startswith(":"):
                if strip_variable_sigil(name) in self.global_variables:
                    raise Exception(f"Variable {name} is already defined")
                self.global_variables[strip_variable_sigil(name)] = Variable(self._evaluate_node(value, local_vars), is_local=False, is_mutable=True)
            case ["defn-mutable", name, value] if name.startswith("$"):
                if strip_variable_sigil(name) in local_vars:
                    raise Exception(f"Variable {name} is already defined")
                local_vars[strip_variable_sigil(name)] = Variable(self._evaluate_node(value, local_vars), is_local=True, is_mutable=True)
            case ["mutate", name, value] if name.startswith(":"):
                ...
                self.global_variables[strip_variable_sigil(name)].value = self._evaluate_node(value, local_vars)
            case ["mutate", name, value] if name.startswith("$"):
                if strip_variable_sigil(name) not in local_vars:
                    raise Exception(f"Variable {name} is not defined")
                if not local_vars[strip_variable_sigil(name)].is_mutable:
                    raise Exception(f"Variable {name} is not mutable")
                local_vars[strip_variable_sigil(name)].value = self._evaluate_node(value, local_vars)
            case ["for-each", name, iterable, body] if name.startswith("$"):
                for item in self._evaluate_node(iterable, local_vars):
                    local_vars[strip_variable_sigil(name)] = Variable(item, is_local=True)
            ...
            case ["lambda", arguments, body]:
                ...
                return lambda *args : self._evaluate_node(body, local_vars | dict(zip(arguments, tuple(Variable(arg, is_mutable=False, is_local=True) for arg in args))))
            case ["list", *items]:
            ...
            case ["range", start, end, step]:
                return tuple(range(self._evaluate_node(start, local_vars), self._evaluate_node(end, local_vars), self._evaluate_node(step, local_vars)))
            case ["range", start, end]:
                return tuple(range(self._evaluate_node(start, local_vars), self._evaluate_node(end, local_vars)))
            case ["range", end]:
            ...
            case _:
                if isinstance(node, str) and (node.startswith(":") or node.startswith("$")):
                    if node.startswith("$") and strip_variable_sigil(node) in local_vars:
                        return local_vars[strip_variable_sigil(node)].value
                    elif node.startswith(":") and strip_variable_sigil(node) in self.global_variables:
